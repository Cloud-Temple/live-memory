...
async function maintCompact(){const s=_ms();if(!s)return;const dry=document.getElementById('mp_dry').checked;showModal('📦 Compacting…','<div class="page-loading">'+(dry?'Scanning…':'Splitting oversized files…')+'</div>',null,null);const r=await callTool('bank_compact',{space_id:s,dry_run:dry});closeModal();showModal('📦 Compact Result',renderPretty('bank_compact',r),'Close',()=>true);}
