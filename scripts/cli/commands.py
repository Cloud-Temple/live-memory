# -*- coding: utf-8 -*-
"""
CLI Click — Scriptable commands for Live Memory.
...
def space_update_rules_cmd(ctx, space_id, rules_file, jflag):
    """📜 Updates a space's rules (admin only).
...
def bank_write_cmd(ctx, space_id, filename, content_file, content, jflag):
    """✏️ Write/replace a bank file (admin, bypasses the LLM).
...
def bank_delete_cmd(ctx, space_id, filename, jflag):
    """🗑️ Delete a bank file + duplicates (admin, irreversible)."""
...
def bank_repair_cmd(ctx, space_id, apply, jflag):
    """🔧 Repair corrupted names (Unicode, duplicates). Dry-run by default.
...
@click.option("--space-ids", default="", help="Authorized spaces (comma-separated)")
@click.option("--expires-in-days", default=0, help="Expiration (0=never)")
...
def token_create_cmd(ctx, name, permissions, space_ids, expires_in_days, email, jflag):
    ...
    Available permissions:
      read                    — Read only
      read,write              — Read + write (notes, consolidation, spaces)
...
def token_update_cmd(
    ...
    _run_tool(
        ...
        "admin_update_token",
        args,
        lambda r: show_success(f"Token updated : {r.get('message', 'OK')}"),
        jflag,
    )
